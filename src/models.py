...
class GNN_classification(torch.nn.Module):
    def __init__(self, label_size, vocabulary_size,embedding_size,num_gnn_layers,num_linear_layer,activation):
        ...
        self.linear_list = ModuleList(linear_list)
        self.linear_ln_list = ModuleList(linear_ln_list)
    ...
    def forward(self, data):
        ...
        # GNN layers
        for conv, ln in zip(self.conv_list, self.conv_ln_list):
            x = conv(x, edges)
            x = ln(x)
        ...
        # linear layers
        for lin, ln in zip(self.linear_list, self.linear_ln_list):
            x = lin(x)
            x = ln(x)
            # x = F.dropout(x, p=0.8, training=self.training)
            x = F.relu(x)
...
class Hyper_classification(torch.nn.Module):
    def __init__(self, label_size, vocabulary_size, edge_arity_dict, embedding_size, num_gnn_layers,
        ...
        conv_ln_list = []
        for i in range(num_gnn_layers):
            hyper_conv_list.append(HyperConv(embedding_size, embedding_size, edge_arity_dict=edge_arity_dict,activation=self.activation))
            conv_ln_list.append(LayerNorm(embedding_size))
    ...
    def forward(self, data):
        ...
        # GNN layers
        for conv, ln in zip(self.hyper_conv_list, self.conv_ln_list):
